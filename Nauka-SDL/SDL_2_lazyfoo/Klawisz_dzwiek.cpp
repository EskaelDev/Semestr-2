	...
	if (e->type == SDL_MOUSEMOTION || e->type == SDL_MOUSEBUTTONDOWN || e->type == SDL_MOUSEBUTTONUP)
	{
		...
		else
		{
			...
			case SDL_MOUSEBUTTONDOWN:
				...
				Mix_PlayChannel(-1, gSplash_click, 0);
				break;
	...
	else
	{
		...
		else
		{
			...
			else
			{
				...
				{
					cout << "SDL_image nie moze byc zainicjalizowany, SDL_image Error: " << IMG_GetError() << endl;
					success = false;
				}
				// Inicjalizacja SDL_mixer 
				if( Mix_OpenAudio( 44100, MIX_DEFAULT_FORMAT, 2, 2048 ) < 0 )
				{
					cout << "SDL_mixer nie moze byc zainicjalizowany, SDL_Mixer Error: " << Mix_GetError() << endl;
					success = false;
				}
			}
		}
...
bool loadMedia()
{
	...
	// Laduje muzyke
	gSplash_click = Mix_LoadWAV("s:/SDL Tutorial/splash_click.wav");
	if (gSplash_click == NULL)
	{
...
void close()
{
	...
	// Zwalnia pamiec z muzyka
	Mix_FreeChunk(gSplash_click);
	gSplash_click = NULL;	
